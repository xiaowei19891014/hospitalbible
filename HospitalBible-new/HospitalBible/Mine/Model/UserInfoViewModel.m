...
+(void)requestUserInfoWithUserId:(NSString *)userId successHandler:(SuccessCallBack)successHandler errorHandler:(ErrorCallBack)errorHandler
{
    ...
    [[ERHNetWorkTool sharedManager] requestDataWithUrl:USER_INFO params:params success:^(NSDictionary *responseObject) {
        if (successHandler) {
            ...
            NSArray *arr = @[detailModel.imgurl,NOTNIL(detailModel.nickname) ,detailModel.idtype,detailModel.idcard,detailModel.sex,detailModel.cellphone,NOTNIL( detailModel.age),detailModel.address,detailModel.height,detailModel.weight,detailModel.email,@"",@"",@"",detailModel.id];
            successHandler(arr);
        }
